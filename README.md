...
A local Web tool for moving selected Codex sessions between provider buckets with preflight checks, complete backups, a tamper-evident audit log, automatic rollback, and conflict-aware restore.
...
2. Select sessions from one source provider and choose the target provider.
3. Run preflight and resolve every critical finding.
4. Read the compatibility and storage-version warnings.
5. Enter `MIGRATE` and execute. Do not interrupt the process.
6. Start Codex with the target provider and verify the session before deleting any backup.
7. To revert, stop Codex again and use **操作与恢复**. Restore is blocked if the session changed after migration.
...
Architecture, source findings, invariants, and limits are documented in [docs/DESIGN.md](docs/DESIGN.md).
