...
`HostFleet` owns inventory, preflight, transfer, archive state, rollback, and restore. A same-remote-host Fork calls official `thread/fork` directly; Move verifies that fork and archives the source, producing a new Session ID without rewriting remote SQLite over SSH. Cross-host Fork stages a hash-verified rollout and uses Codex's experimental `thread/fork.path` request with a target Provider and cwd. Cross-host Move is verified target creation followed by source archive, never source deletion. Archive and Unarchive use the same selected-host adapter, so local and remote calls share the official app-server mutation, writer-lock checks, local audit snapshots, and rollback rules.
